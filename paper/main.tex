...
\end{equation}
Substituting for $h'(t)$ with \eqref{eq:SSMlatent} (the $\mathbf{D}$ matrix can be considered as a skip connection and is ignored in this calculation) and discretizing the discretized parameters of the model, the "continuous parameters" $(\Delta, \mathbf{A}, \mathbf{B})$ are transformed to "discrete parameters" $(\mbfb{A}, \mbfb{B})$
...
%% This was previosuly here, didn't delete yet
...
\begin{algorithm}
    ...
    \end{algorithmic}
    \end{algorithm}
Multiple of the S6 blocks are stacked to form the Mamba architecture, similarly to how Transformers stack their self-attention blocks.

%% EDITS STOPPED HERE -- wanting to add more on Mamba (scan operation details, hardware aware computation, kernel fusion, recomputation of loss?)
